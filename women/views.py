...
#     new_women = {'women': women, 'menu': menu}
...
def detail_women(request, women_slug):
        ...
        return render(request, 'detail-women.html', context=new_women)
...
#
#
def delete_women(request, women_slug):
    if request.method == "POST":
        Women.objects.filter(slug=women_slug).delete()
        return redirect('index')